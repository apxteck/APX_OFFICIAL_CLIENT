...
export function PasswordForm({ formData, handleInputChange, handleSave, isSubmitting, message }: PasswordFormProps) {
  return (
    <div className="bg-white dark:bg-[#111] p-6 md:p-8 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm space-y-8">
      ...
      <form className="space-y-6" onSubmit={handleSave}>
        ...
        {message && (
          <div className={`p-4 rounded-xl flex items-center gap-2 max-w-md ${message.type === 'success' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
        ...
        <div className="pt-4 flex items-center gap-4">
          ...
          <Link href="/customer/settings" className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 font-bold px-4 py-3 transition-colors flex items-center justify-center min-h-[44px]">
            Cancel
          </Link>
        </div>
      </form>
