    ...
    <motion.div variants={container} initial="hidden" animate="show" className="w-full max-w-3xl mx-auto px-4 sm:px-6 md:px-8 pt-4 pb-safe space-y-8">
      <motion.div variants={item} className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Manage how and when you receive alerts and updates.</p>
        </div>
      </motion.div>
      ...
      <motion.div variants={item} className="space-y-6">
        ...
        {message && (
          <div className={`p-4 rounded-xl flex items-center gap-2 ${message.type === 'success' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
