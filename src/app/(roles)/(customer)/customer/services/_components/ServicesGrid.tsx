...
export function ServicesGrid() {
  ...
  if (isLoading) {
    return (
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => (
           <div key={i} className="bg-white dark:bg-[#111] p-6 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm animate-pulse flex flex-col h-[250px]">
  ...
  return (
    <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <Link href="/customer/services/new" className="bg-gradient-to-br from-gray-50 to-white dark:from-[#151515] dark:to-[#111] p-6 rounded-3xl border-2 border-dashed border-gray-200 dark:border-white/10 flex flex-col items-center justify-center text-center hover:border-cyan-500/50 hover:shadow-[0_0_30px_rgba(6,182,212,0.15)] transition-all cursor-pointer min-h-[250px] group">
        ...
        <h3 className="font-bold text-gray-900 dark:text-white mb-2 text-lg">Explore New Services</h3>
        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-[200px]">Discover more ways we can help your business grow and scale.</p>
      </Link>
    </motion.div>
  );
}
