...
import { 
  PlusCircle, ArrowLeft, Edit, Trash2, ChevronUp, ChevronDown, ListTodo, X 
} from "lucide-react";
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {formData.options.map((opt, i) => (
                          <div key={i} className="flex items-center gap-2 bg-gray-100 dark:bg-black border border-gray-200 dark:border-white/10 px-3 py-1.5 rounded-lg text-sm text-gray-700 dark:text-gray-300">
                            {opt}
                            <button type="button" onClick={() => removeOption(i)} className="min-w-[44px] min-h-[44px] flex items-center justify-center text-gray-400 hover:text-red-500">
                              <X size={14} />
                            </button>
                          </div>
                        ))}
