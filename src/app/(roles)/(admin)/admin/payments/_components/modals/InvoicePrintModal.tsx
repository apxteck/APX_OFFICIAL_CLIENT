              ...
              <div className="relative z-10 flex flex-col h-full print:h-full">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-stretch gap-6 mb-12">
                  ...
                  <div className="text-left sm:text-right w-full sm:w-auto flex flex-col justify-between">
                    ...
                    <div className="space-y-1.5 mt-auto sm:text-right">
                      <p className="text-sm font-semibold text-gray-900 flex justify-between sm:justify-end gap-4"><span>Invoice Number:</span> <span className="text-gray-500 font-medium">INV-{payment.id}</span></p>
                      <p className="text-sm font-semibold text-gray-900 flex justify-between sm:justify-end gap-4"><span>Date Issued:</span> <span className="text-gray-500 font-medium">{payment.paidAt ? format(new Date(payment.paidAt), "MMM dd, yyyy") : format(new Date(payment.createdAt), "MMM dd, yyyy")}</span></p>
                      <p className="text-sm font-semibold text-gray-900 flex justify-between sm:justify-end gap-4"><span>Transaction ID:</span> <span className="text-gray-500 font-medium">{payment.transactionId || 'N/A'}</span></p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-12 bg-gray-50/80 p-8 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-900 mb-1">{payment.customer.fullName}</p>
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                      <tr className="group hover:bg-gray-50/50 transition-colors">
                        <td className="py-6 px-8">
                          ...
                          <p className="text-sm font-medium text-gray-500 mt-1">Service Request #{payment.serviceRequestId}</p>
                          {payment.invoiceNote && (
                              <div className="mt-5 bg-emerald-50 border border-emerald-100/50 text-emerald-800 text-sm p-4 rounded-2xl flex items-start gap-3">
                                ...
                                <div>
                                  ...
                                  <span className="text-emerald-700/90 leading-relaxed">{payment.invoiceNote}</span>
                                </div>
                              </div>
                          )}
                        </td>
                        <td className="py-6 px-8 text-right align-top">
                          <span className="text-xl font-bold text-gray-900">₹{payment.amountPaid || payment.negotiatedAmount}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-end gap-6 mb-12 mt-6">
                  ...
                  <div className="w-full sm:w-[380px] bg-gray-50 p-8 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden">
                    <div className="flex justify-between items-center py-2.5 text-sm text-gray-600">
                      ...
                      <span className="font-semibold text-gray-900 text-base">₹{payment.amountPaid || payment.negotiatedAmount}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      <span className="text-3xl font-black text-emerald-600 tracking-tight">₹{payment.amountPaid || payment.negotiatedAmount}</span>
                    </div>
                  </div>
                </div>
