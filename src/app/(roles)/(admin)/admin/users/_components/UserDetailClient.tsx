...
export function UserDetailClient({ initialUser, initialPermissions, initialRoles }: Props) {
  const {
    router, user, roles, permissions, activeTab, setActiveTab,
    isEditing, setIsEditing, isSaving,
    isEditingPerms, setIsEditingPerms, isSavingPerms,
    editedPerms, toast, setToast,
    rejectingDocId, setRejectingDocId, rejectNote, setRejectNote,
    isUpdatingDoc, form, profilePicturePreview,
    setProfilePictureFile, setProfilePicturePreview,
    handleChange, handleCancel, handleSave, handleToggleActive,
    handleUpdateDocumentStatus, handlePermToggle, handleCancelPerms,
    handleSavePerms, handleRevokeModule, handleResetAllPerms
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-xl p-3.5 text-center border border-gray-100 dark:border-white/[0.04]">
                <p className="text-2xl font-black text-gray-900 dark:text-white">{user.documents?.length || 0}</p>
                ...
                <motion.div key="documents" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 8 }} transition={{ duration: 0.2 }} className="p-6 md:p-8">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-white/5 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/10">
                      {user.documents?.length || 0} Document{user.documents?.length !== 1 ? "s" : ""}
                    </span>
                  </div>
