...
export function RoleDetailClient({ initialRole, initialPermissions }: Props) {
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto pb-safe pb-12 px-4 sm:px-6 md:px-8">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-3">
            Role Configuration:{' '}
            <span className="text-indigo-600 dark:text-indigo-400">{role.name}</span>
            {isProtected && (
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1">
            Manage details and default module permissions.
          </p>
        </div>
      </div>
      ...
      {(error || success) && (
        <div
          className={`p-4 rounded-xl text-sm font-bold border ${error ? 'bg-red-50 text-red-600 border-red-200' : 'bg-emerald-50 text-emerald-600 border-emerald-200'}`}
        >
          {error || success}
        </div>
      )}
                ...
                >
                  <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Update the basic information of this role.
                    </p>
                  </div>
                  ...
                  {isProtected && (
                    <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl flex gap-3">
                      ...
                      <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                        This is a system-protected role. Its core details cannot be modified or
                        deleted to ensure system stability.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="p-6 md:p-8 border-b border-gray-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Configure CRUD access defaults for any user assigned to this role.
                      </p>
                    </div>
                    <button
                      ...
                      <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                          {permissions.map((perm) => (
                            ...
                            >
                              <td className="px-6 py-4 font-bold text-gray-900 dark:text-white text-sm font-mono">
                                {perm.module}
                              </td>
                              {(['canCreate', 'canRead', 'canUpdate', 'canDelete'] as const).map(
                                (field) => (
                                  <td key={field} className="px-6 py-4 text-center">
                                    <div className="min-w-[44px] min-h-[44px] flex items-center justify-center w-full">
                                      <input
                                        type="checkbox"
                                        checked={perm[field]}
