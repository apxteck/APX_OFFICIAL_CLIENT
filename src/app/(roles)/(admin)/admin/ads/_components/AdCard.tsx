...
export function AdCard({ ad, onEdit, onDelete, onToggleStatus }: AdCardProps) {
  return (
    <div className="bg-white dark:bg-[#111111] rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-md transition-all flex flex-col group relative overflow-hidden">
      ...
      >
        <div
          className={`w-8 h-4 rounded-full p-0.5 transition-colors ${ad.isActive ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-700'}`}
        >
          <div
            className={`w-3 h-3 bg-white rounded-full shadow-sm transform transition-transform ${ad.isActive ? 'translate-x-4' : 'translate-x-0'}`}
          ></div>
        </div>
      </button>
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4 font-mono">
          <span className="bg-gray-100 dark:bg-[#222] px-2 py-0.5 rounded">{ad.placement}</span>
        </div>
        ...
        {ad.targetUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 font-medium mb-4 hover:underline break-all"
          >
