...
export async function generateMetadata({ params }: { params: { orderId: string } }): Promise<Metadata> {
  ...
  return {
    ...
    description: 'Securely process your payment for APXTeck\'s premium IT services, web development, and digital marketing solutions. Enjoy a seamless checkout experience.',
    alternates: {
...
export default function CheckoutPage({ params }: { params: { orderId: string } }) {
  ...
  return (
    <div className="min-h-dvh bg-[url('/grid-bg.svg')] bg-gray-50 dark:bg-gray-950 flex flex-col items-center justify-center p-4 relative w-full overflow-x-hidden pt-safe pb-safe">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[500px] bg-gradient-to-b from-indigo-500/10 via-purple-500/5 to-transparent dark:from-indigo-500/20 dark:via-purple-500/10 dark:to-transparent pointer-events-none blur-3xl" aria-hidden="true"></div>
      <div className="absolute -top-40 -right-40 w-96 h-96 bg-indigo-500/20 rounded-full blur-3xl opacity-50 dark:opacity-30 mix-blend-multiply" aria-hidden="true"></div>
      <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl opacity-50 dark:opacity-30 mix-blend-multiply" aria-hidden="true"></div>
