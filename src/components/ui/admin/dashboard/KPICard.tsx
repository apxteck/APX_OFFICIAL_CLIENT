  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <p className="text-sm font-bold text-gray-500 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors">{data.title}</p>
      ...
      <div className="flex items-end justify-between relative z-10">
        ...
        {data.percentage !== undefined && (
          <div className={`flex items-center gap-1 text-sm font-bold px-2.5 py-1.5 rounded-xl shadow-sm backdrop-blur-sm ${
            isUp ? "text-emerald-700 bg-emerald-100/80 border border-emerald-200/50 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20" :
            isDown ? "text-red-700 bg-red-100/80 border border-red-200/50 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20" :
      ...
      {data.sparklineData && (
        <div className="absolute bottom-0 left-0 right-0 h-16 opacity-20 group-hover:opacity-60 transition-opacity duration-500 pointer-events-none">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-current to-transparent opacity-10" 
               style={{ color: isUp ? "#10B981" : isDown ? "#EF4444" : "#6366f1" }} />
               
          <svg viewBox="0 0 100 20" preserveAspectRatio="none" className="w-full h-full relative z-10">
            <polyline
