...
export function ContactPageSection({ services }: ContactPageSectionProps) {
  return (
    <section aria-labelledby="contact-heading" className="py-12 md:py-16 max-w-7xl mx-auto px-4 sm:px-6 w-full">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start w-full">
        {/* Left Column: Info Cards & Map */}
        <div className="lg:col-span-5 flex flex-col gap-8 w-full">
          <header className="flex flex-col gap-4">
            <h1 id="contact-heading" className="text-3xl sm:text-4xl font-extrabold tracking-tight text-foreground w-full">
              Get in Touch
            </h1>
          ...
          <address className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 not-italic w-full">
            <div className="p-4 sm:p-5 rounded-[1.25rem] bg-foreground/[0.01] border border-glass-border flex gap-3 w-full group transition-colors hover:bg-foreground/[0.03]">
              <Mail className="w-5 h-5 text-accent shrink-0 mt-0.5 group-hover:scale-110 transition-transform" aria-hidden="true" role="presentation" />
              <div className="flex flex-col">
                <h2 className="font-bold text-sm">Email Our Tech Experts</h2>
                <a href="mailto:info@apxteck.com" className="text-xs sm:text-sm text-foreground/60 mt-1 hover:text-accent transition-colors min-h-[44px] flex items-center -ml-2 px-2 -my-2" aria-label="Email APXTeck at info@apxteck.com">
                  info@apxteck.com
                </a>
              </div>
            </div>
            <div className="p-4 sm:p-5 rounded-[1.25rem] bg-foreground/[0.01] border border-glass-border flex gap-3 w-full group transition-colors hover:bg-foreground/[0.03]">
              <Phone className="w-5 h-5 text-purple-400 shrink-0 mt-0.5 group-hover:scale-110 transition-transform" aria-hidden="true" role="presentation" />
              <div className="flex flex-col">
                <h2 className="font-bold text-sm">Call Our IT Office</h2>
                <a href="tel:+919405282582" className="text-xs sm:text-sm text-foreground/60 mt-1 hover:text-accent transition-colors min-h-[44px] flex items-center -ml-2 px-2 -my-2" aria-label="Call APXTeck at +91 94052 82582">
                  +91 94052 82582
                </a>
              </div>
            </div>
            <div className="p-4 sm:p-5 rounded-[1.25rem] bg-foreground/[0.01] border border-glass-border flex gap-3 sm:col-span-2 lg:col-span-1 w-full group transition-colors hover:bg-foreground/[0.03]">
              <MapPin className="w-5 h-5 text-pink-500 shrink-0 mt-0.5 group-hover:scale-110 transition-transform" aria-hidden="true" role="presentation" />
              <div className="flex flex-col">
